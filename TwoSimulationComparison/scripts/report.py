...
mean_legsmetricsdf = pd.read_csv(os.path.join(
    processed_path, "legssystemdf_mean.csv"))
median_legsmetricsdf = pd.read_csv(os.path.join(
    processed_path, "legssystemdf_median.csv"))
...
for key, figure in data_figures.items():
    try:
        data_figures[key] = [dp.Media(file=os.path.join(processed_path, figure[0])), dp.Media(
            file=os.path.join(processed_path, figure[1]))]
    except:
        data_figures[key] = [
            dp.Media(file=os.path.join(processed_path, figure[0])), ]
...
if __name__ == "__main__":
    report_content = dp.View(
        ...
        dp.Text(
            'Active links have a flow greater than 0. Congested links have v/c higher than 0.75 .'),
        dp.Table(activitydf),
