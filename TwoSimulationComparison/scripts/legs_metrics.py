...
def leg_metrics():
    ...
    def x(sim, time_min): return len(sim[sim['congested_ttmin'] > time_min])
    sim1_time5, sim2_time5 = x(
        leg_simulation_one, 5 * 60), x(leg_simulation_two, 5 * 60)
    sim1_time10, sim2_time10 = x(
        leg_simulation_two, 10 * 60), x(leg_simulation_two, 10 * 60)

    # Mean
    metrics = ['Number of Trips', 'Average Trip Distance(miles)', 'Average Travel Time(min)',
               'Average Trip Delay(min)', 'Number of trips with TT > 5 hours', 'Number of trips with TT > 10 hours']

    sim1_metrics = ["{:,}".format(len(leg_simulation_one)),
                    np.round(util.get_mean(
                        leg_simulation_one['total distance (m)']) * 0.000621371, decimals=1),
    ...
    sim2_metrics = ["{:,}".format(len(leg_simulation_two)),
                    np.round(util.get_mean(
                        leg_simulation_two['total distance (m)']) * 0.000621371, decimals=1),
                    util.get_mean(leg_simulation_two['congested_ttmin']),
                    util.get_mean(leg_simulation_two['delay_ttmin']),
                    int(sim2_time5),
                    int(sim2_time10)]

    legssystemdf = pd.DataFrame(list(zip(metrics, sim1_metrics, sim2_metrics)), columns=[
                                'Metric', sim_one_name, sim_two_name])
    print(legssystemdf)
    ...
    sim1_metrics = [np.round(util.get_median(leg_simulation_one['total distance (m)']) * 0.000621371, decimals=1),
                    ...
                    np.round(
                        util.p95(leg_simulation_one['total distance (m)']) * 0.000621371, decimals=1),
    ...
    sim2_metrics = [np.round(util.get_median(leg_simulation_two['total distance (m)']) * 0.000621371, decimals=1),
                    ...
                    np.round(
                        util.p95(leg_simulation_two['total distance (m)']) * 0.000621371, decimals=1),
    ...
    print(legssystemdf)
    legssystemdf.to_csv(
        processed_path / f"legssystemdf_median.csv", index=False)
