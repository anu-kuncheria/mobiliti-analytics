...
data_simulations = [
    ...
    [sim_two_name, simulation_two_flow,
     simulation_two_speed, simulation_two_fuel]
]
...
def system_metrics_table():
    ...
    systemdf.loc[:, 'VMT (thousand miles)'] = systemdf['VMT (thousand miles)'].map(
        '{:,}'.format)  # to format numbers with commas
    systemdf.loc[:, 'VHD'] = systemdf['VHD'].map('{:,}'.format)
    systemdf.loc[:, 'Fuel Consumption (thousand gallons)'] = systemdf['Fuel Consumption (thousand gallons)'].map(
        '{:,}'.format)
...
def system_metrics_functionalclass():
    ...
    # writing bar plots
    util.dodged_barplot(vmt_params[0], vmt_params[1], 'VMT', sim_one_name,
                        sim_two_name, 'Functional class', 'VMT (in million miles)', processed_path)
    util.dodged_barplot(vhd_params[0], vhd_params[1], 'VHD', sim_one_name,
                        sim_two_name, 'Functional class', 'VHD (in thousand hours)', processed_path)
    util.dodged_barplot(fuel_params[0], fuel_params[1], 'Fuel', sim_one_name, sim_two_name,
                        'Functional class', 'Fuel Consumption (in thousand gallons)', processed_path)
