...
def networkGeom(links, nodes, crs="epsg:4326"):
    ...
    links['nref_long'] = links['NREF_IN_ID'].map(
        nodes.set_index('NODE_ID')['LON'])
    links['geometry'] = links.apply(lambda x: geom.LineString(
        [(x['ref_long'], x['ref_lat']), (x['nref_long'], x['nref_lat'])]), axis=1)
...
def read_file(path):
    ...
    file.drop(file.columns[len(file.columns) - 1], axis=1,
              inplace=True)  # dropping the last Nan column
    return file


def results_city_len(flow, links):
    flow_c = flow.merge(links, left_on='link_id',
...
def vmtfc(flow_len):
    ...
    for fc in fc_att:
        ...
        vmt = np.sum(flow_sub.iloc[:, 1:97].sum(
            axis=1) * 15 * 60 * flow_sub.loc[:, 'LENGTH(meters)'] * 0.000621371)
...
def fuel_gallons_fc(fuel_len):
    ...
    for fc in fc_att:
        ...
        fuelga = fuel_sub.loc[:, '00:00': '23:45'].sum(
        ).sum() * 0.264172  # gallons
...
def generateColumnNamesFigures():
    """
    Generates column names for figures.
    """
    result = ['00:00']
    minc, hourc = 0, 0
    for i in range(0, 95):
...
def plot_flow_speed(linkid, *data, flows=True, attr="", processed_path):
    ...
    if flows == True:
        ...
        for f, l, c in zip(fdf, l, c):
            ax.plot(f.loc[linkid, "00:00":"23:45"].values *
                    15 * 60, color=c, label=l, linewidth=1.5)

        ax.text(
            30, 100, 'Capacity(v/hr): {}'.format(int(fdf[0].loc[linkid, 'CAPACITY(veh/hour)'])))
        ax.axhline(y=int(fdf[0].loc[linkid, 'CAPACITY(veh/hour)'] / 4),
                   color='slategray', linestyle='dashed', linewidth=2)
        ...
        for f, l, c in zip(fdf, l, c):
            ax.plot(f.loc[linkid, "00:00":"23:45"].values *
                    2.23694, color=c, label=l, linewidth=1.5)

        ax.text(5, 20, 'Free Speed in Mobiliti(mph): {}'.format(
            int(fdf[0].loc[linkid, 'SPEED_KPH'] * 0.621371)))
        ax.axhline(y=fdf[0].loc[linkid, 'SPEED_KPH'] * 0.621371,
                   color='slategray', linestyle='dashed', linewidth=2)
        ax.set_xlabel("Time of day")
...
def dodged_barplot(values1, values2, title, label1, label2, xlabel, ylabel, processed_path):
    ...
    rects2 = ax.bar(ind + width, values2, width,
                    color='seagreen', label=label2)
...
def preprocess_legs(legs_path):
    ...
    l1['delay_ttmin'] = l1['delaytt'].dt.hour * 60 + \
        l1['delaytt'].dt.minute + l1['delaytt'].dt.second / 60
    return l1
...
def plotscatter(x, y, z, title, xlabel, ylabel, savename):
    ...
    sns.scatterplot(x=x, y=y, hue=z, figure=fig1, palette=[
                    'green', 'orange', 'brown', 'dodgerblue', 'red'], legend='full')
...
def congestion_metrics_time_vc(df, timecolumn):
    df_sub = df[[timecolumn, 'FUNC_CLASS', 'SPEED_KPH',
                 'LENGTH(meters)', 'CAPACITY(veh/hour)']]
    df_sub['vc'] = df_sub[timecolumn] * 3600 / df_sub['CAPACITY(veh/hour)']
    congestedmiles = df_sub[df_sub['vc'] >=
                            0.75]['LENGTH(meters)'].sum() * 0.000621371
