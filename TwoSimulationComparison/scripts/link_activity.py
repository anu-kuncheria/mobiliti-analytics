...
if __name__ == '__main__':
    ...
    flow_filter = pd.merge(simulation_one_flow[['link_id', '08:00', '13:00', '16:00', 'FUNC_CLASS']], simulation_two_flow[[
                           'link_id', '08:00', '13:00', '16:00']], left_on='link_id', right_on='link_id', how='left')
    speed_filter = pd.merge(simulation_one_speed[['link_id', '08:00', '13:00', '16:00', 'FUNC_CLASS']], simulation_two_speed[[
                            'link_id', '08:00', '13:00', '16:00']], left_on='link_id', right_on='link_id', how='left')
    ...
    for time, var in vc_params.items():
        var[0] = util.congestion_metrics_time_vc(
            df=simulation_one_flow, timecolumn=time)
        var[1] = util.congestion_metrics_time_vc(
            df=simulation_two_flow, timecolumn=time)
        var[2] = util.congestion_metrics_time_speed(
            df=simulation_one_speed, timecolumn=time)
        var[3] = util.congestion_metrics_time_speed(
            df=simulation_two_speed, timecolumn=time)

    sim1_congestedmiles_8am_vc, sim2_congestedmiles_8am_vc, sim1_congestedmiles_8am_speedratio, sim2_congestedmiles_8am_speedratio = vc_params[
        '08:00']
    sim1_congestedmiles_4pm_vc, sim2_congestedmiles_4pm_vc, sim1_congestedmiles_4pm_speedratio, sim2_congestedmiles_4pm_speedratio = vc_params[
        '16:00']
    sim1_congestedmiles_8pm_vc, sim2_congestedmiles_8pm_vc, sim1_congestedmiles_8pm_speedratio, sim2_congestedmiles_8pm_speedratio = vc_params[
        '20:00']
    ...
    activitydf[sim_one_name] = [
        int(simulation_one_flow[simulation_one_flow['08:00']
            > 0]['LENGTH(meters)'].sum() * 0.000621371),
        int(simulation_one_flow[simulation_one_flow['16:00']
            > 0]['LENGTH(meters)'].sum() * 0.000621371),
    ...
    activitydf[sim_two_name] = [
        int(simulation_two_flow[simulation_two_flow['08:00']
            > 0]['LENGTH(meters)'].sum() * 0.000621371),
        int(simulation_two_flow[simulation_two_flow['16:00']
            > 0]['LENGTH(meters)'].sum() * 0.000621371),
    ...
    activitydf.loc[:, sim_one_name] = activitydf[sim_one_name].map(
        '{:,d}'.format)
    activitydf.loc[:, sim_two_name] = activitydf[sim_two_name].map(
        '{:,d}'.format)
